...
Sample query = proximity_phrase(sentence, 'pulmonary edema', bilateral, 4, False/True)

"""


def proximity_phrase(sentence,word1,word2,number,order=True):
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        if len(sys.argv) == 6:
            if sys.argv[5] == 'False':
                print(proximity_phrase(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],False))
            else:
                print('With default as True')
                print(proximity_phrase(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4]))
        else:
            print(proximity_phrase(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4]))

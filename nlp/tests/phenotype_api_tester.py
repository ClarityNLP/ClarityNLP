import data_access.memory_data
...
def two_calls_test():
    ...
    reports = [{
        ...
        'report_date': "2022-06-24T15:02:43.378272Z",
        'subject': 'TEST_SUBJECT_1234',
        'report_text': "fever vomiting, nausea, with sepsis"
    }]
    source_id = str(uuid.uuid4())
    data_access.memory_data.load_buffer(source_id, reports)

    nlpql = '''
        debug;

        // Phenotype library name
        phenotype "Septic features" version "2";
    ...
    status = post_nlpql(nlpql, source_id, False)
    print(status)
    # job_id = status.get('job_id')

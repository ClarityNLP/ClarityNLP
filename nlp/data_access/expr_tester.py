...
###############################################################################
def _compare_context_sets(context_field, eval_results, mongo_results):
    """
     eval_results: list of (expr_obj, docs) tuples
    mongo_results: list of documents
    """
    ...
    for expr_obj, docs in eval_results:
        for doc in docs:
            ...
            eval_set.add(doc[context_field])

    for doc in mongo_results:
        assert context_field in doc
        mongo_set.add(doc[context_field])
...
###############################################################################
def _run_self_tests(job_id,
                    context_var,
                    mongo_obj):   # mongo collection object
    """
    ...
    #
    # evaluate expressions and check with independent MongoDB query
    #
    
    # all temperature measurements
    expr = 'Temperature'
    eval_results = _run_selftest_expression(job_id, cf, expr, mongo_obj)
    mongo_results = mongo_obj.find(
        {
            'job_id':job_id,
            'nlpql_feature':'Temperature'
        })
    if not _compare_context_sets(cf, eval_results, mongo_results):
        return False
    ...
    # all temperature and lesion measurements
        ...
        {
            {
                ...
                [
                    {'job_id':job_id},
                    {'nlpql_feature':{'$eq':'Temperature'}},
                    {'nlpql_feature':{'$eq':'Lesion'}}
                ]
            }
        })
    if not _compare_context_sets(cf, eval_results, mongo_results):

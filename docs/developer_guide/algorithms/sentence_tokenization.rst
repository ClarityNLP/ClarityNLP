...
Here is a before and after example illustrating the tokenization problems
discussed above. The data is taken from one of the reports in the MIMIC data
set.
...
inspecting the sentences generated by the tokenizer. The input data must be a
JSON-formatted file with the proper ClarityNLP fields. This file can be
generated by querying SOLR for the reports of interest and dumping
the results in JSON format. The sentence tokenization module will read the
input file, perform the sentence tokenization as described above, and write the
sentences to stdout. Help for the command line interface can be obtained by
